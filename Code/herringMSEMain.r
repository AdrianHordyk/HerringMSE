...
nsim <- 200 # number of simulations - increase later and re-run script 
...
Overwrite <- TRUE # should the saved object be written over if it already exists? 
...
# test<- runMSE(new("OM", Albacore, Generic_fleet, Perfect_Info))
# test@OM
...
fixed0.05 <- makeMinMaxMP(minB=0, maxB=1, PercCap=0.05) # fixed harvest rate at 5% 
fixed0.1 <- makeMinMaxMP(minB=0, maxB=1, PercCap=0.1) # fixed harvest rate at 10% 
fixed0.15 <- makeMinMaxMP(minB=0, maxB=1, PercCap=0.15) # fixed harvest rate at 15%
